// light

const int primaryColor = 0xFF2196F3;
const int secondaryColor = 0xFF3F51B5;
const int backgroundColor = 0xFFF4F9FD;
const int background2Color = 0xFF2155A3;

const int shadowColor = 0xFFC4CBD6;

const int chartBar1Color = 0xFF962DFF;
const int chartBar2Color = 0xFFC893FD;
const int chartBar3Color = 0xFFE0C6FD;
const int chartBar4Color = 0xFFF0E5FC;
const int chartBar5Color = 0xFFF2F1FF;

const int chartBarBgColor = 0xFFF3E5F5;
const int chartLineColor = 0xFFF44336;
const int chartAreaColor = 0xFFFF9800;

const int grey1Color = 0xFFE6EDF5;
const int grey2Color = 0xFF7D8592;
const int grey3Color = 0xFF8D94A0;
const int grey4Color = 0xFFD8E0F0;
const int grey5Color = 0xFF7D8593;
const int grey6Color = 0xFFF4F1FD;
const int grey7Color = 0xFF91929E;
const int grey8Color = 0xFFF3F8FC;
const int grey9Color = 0xFF615E83;
const int grey10Color = 0xFFE1E7F3;
const int grey11Color = 0xFFD8DDE6;
const int grey12Color = 0xFF979797;

const int whiteColor = 0xFFFFFFFF;
const int blackColor = 0xFF000000;

const int greenPercentColor = 0xFF00FF73;

const int blue2Color = 0xFF0A1629;
const int blue3Color = 0xFF3F8CFF;

const int bluePeriodColor = 0xFF2634CE;
const int redPeriodColor = 0xFFFF3C00;

const int redColor =0xFFF65160;

const int purpleColor = 0xFF6D5DD3;


//dark
